...
var area2text = ["北京", "天津", "河北", "山西", "内蒙古", "辽宁", "吉林", "黑龙江", "上海", "江苏", "浙江", "安徽", "福建", "江西", "山东", "河南", "湖北", "湖南", "广东", "广西", "海南", "重庆", "四川", "贵州", "云南", "西藏", "陕西", "甘肃", "青海", "宁夏", "新疆"];
...
function data_fmt(json) {
	...
	for (var i = 0; i < json.length; i++) {
		var area = json[i][0];
		var subarea = json[i][1];
		json[i][0] = `<span class="badge rounded-pill bg-${type2color[json[i][2]]}">${type2text[json[i][2]]}</span>`;
		json[i][1] = `<a href="#/shop/${i}/">${json[i][3]}</a>`;
...
function app_btn_click(type, id) {
	//const name = window.list[id][4];
	const lng = window.list[id][5];//123.370888
	const lat = window.list[id][6];//41.792397
	let url = '';
	const os = get_os();
	if (os === 0) {//web
		if (type === 0) {//baidu
...
function display_map() {
	...
	if (window.list.length > 0 &&
		document.getElementById('page_map').innerText == "") {
		initMap();
	}
}
function initMap() {
	...
	marker.on("click", function (evt) {
		...
		//console.log(evt);
		var info = window.list[evt.cluster.geometries[0].id]
...
(function () {
	...
	$.getJSON("data_all.json", function (data) {
